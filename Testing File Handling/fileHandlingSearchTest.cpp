...
void searchAndDisplayLineNumbers(const std::string & filename, const std::string & searchStr) {
  ...
  while (std::getline(file, line)) { // Read each line from the file
    if (line.find(searchStr) != std::string::npos) { // Check if the search string is found in the line
      ...
      std::cout << line << std::endl; // Grabbing the line where I wrote it
    }
  ...
  if (!lineNumbers.empty()) { // Check if any line numbers were stored
    std::cout << "String \"" << searchStr << line << "\" found at line number(s): "; // Display a message indicating the search string was found
    ...
    std::cout << "String \"" << searchStr << line << "\" not found in the file." << std::endl; // Display a message indicating the search string was not found
  }
}

int main() {
  std::string search;
  std::cin >> search; // chanaged shit here
  std::string filename = "pswdHolderTEST.txt"; // File to search
  displayFileContent("new_test.txt"); // Display content of "new_test.txt"
  std::cout << std::endl; // Output a newline for formatting

  std::string searchStr = search; // String to search
